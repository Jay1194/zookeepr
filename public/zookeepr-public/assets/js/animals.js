...
const getAnimals = (formData = {}) => {
  ...
  //From there, the object formData will be passed through the Object.entries() method to create query parameters. Because all that work is done for us, all we have to do is make the request using queryUrl! 
  ...
  .then(response => {
    //when using fetch() we have to check to see if the ok property in the response is true or false -  If there's an error, we just alert the user that there's something wrong
    if (!response.ok) {
...
//handleGetAnimalsSubmit() function on form submission. This function will gather all of the form input data and package it as an object to send to the getAnimals() function as the formData argument.
